...
if ($method === 'POST' && $action === 'login') {
    try {
        ...
        $pdo = getDB();
        if ($pdo) {
            $stmt = $pdo->prepare("SELECT * FROM users WHERE email = ? AND is_banned = FALSE");
            $stmt->execute([$email]);
            $user = $stmt->fetch();
            ...
            if ($user && verifyPassword($password, $user['password'])) {
                ...
                $isAdmin = ($user['role'] ?? 'user') === 'admin' || $email === ADMIN_EMAIL;
                sendResponse([
                    'success' => true,
                    'isAdmin' => $isAdmin,
                    'user' => $user
...
if ($method === 'POST' && $action === 'register') {
    try {
        ...
        // Insert new user
        $stmt = $pdo->prepare("
            INSERT INTO users (name, email, password, role, total_points, challenges_solved)
            VALUES (?, ?, ?, 'user', 0, 0)
        ");
